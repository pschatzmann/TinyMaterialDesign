 ...
 * desktop backend so it's clickable with a real mouse - no touch hardware
 * needed to exercise the whole interaction model (see TouchDriverSDL).
 */
...
constexpr size_t kHeight = 1040;

Surface<RGB565> surface(kWidth, kHeight, FontRGB565);
DisplayDriverSDL<RGB565> driver(kWidth, kHeight);
DeviceOutput<RGB565> display(driver);
...
RadioButton<RGB565> radioA(Bounds(16, 292, 24, 24), true);
RadioButton<RGB565> radioB(Bounds(90, 292, 24, 24));
RadioButton<RGB565> radioC(Bounds(164, 292, 24, 24));
Label<RGB565> radioALabel(Bounds(44, 292, 30, 24), "A");
Label<RGB565> radioBLabel(Bounds(118, 292, 30, 24), "B");
Label<RGB565> radioCLabel(Bounds(192, 292, 30, 24), "C");

// --- Slider ---------------------------------------------------------------
Slider<RGB565> volumeSlider(Bounds(16, 336, 200, 24), 0.0f, 100.0f, 40.0f);
Label<RGB565> volumeValueLabel(Bounds(224, 332, 100, 24), "40");

// --- Buttons ---------------------------------------------------------------
Button<RGB565> filledButton(Bounds(16, 368, 92, 40), "Filled", ButtonVariant::kFilled);
Button<RGB565> tonalButton(Bounds(116, 368, 92, 40), "Tonal", ButtonVariant::kTonal);
Button<RGB565> outlinedButton(Bounds(216, 368, 92, 40), "Outlined", ButtonVariant::kOutlined);
Button<RGB565> textButton(Bounds(16, 416, 92, 40), "Text", ButtonVariant::kText);
Button<RGB565> elevatedButton(Bounds(116, 416, 92, 40), "Elevated", ButtonVariant::kElevated);

// --- Progress indicators ---------------------------------------------------
LinearProgressIndicator<RGB565> linearProgress(Bounds(16, 472, 200, 8), 0.0f, /*indeterminate=*/true);
CircularProgressIndicator<RGB565> circularProgress(Bounds(232, 456, 40, 40), 0.0f,
                                                  /*indeterminate=*/true);

// --- Chips -------------------------------------------------------------
Chip<RGB565> chipA(Bounds(16, 508, 90, 32), "Filter A", /*selectable=*/true, /*selected=*/true);
Chip<RGB565> chipB(Bounds(114, 508, 90, 32), "Filter B", /*selectable=*/true);

// --- Divider + card ---------------------------------------------------
Divider<RGB565> divider(Bounds(16, 552, kWidth - 32, 2));
Card<RGB565> infoCard(
    Bounds(16, 564, kWidth - 32, 140), "TinyMaterialDesign",
...
bool playing = false;
Label<RGB565> mediaLabel(Bounds(16, 712, 200, 24), "Now Playing", TypographyRole::kTitle);
...
Label<RGB565> gridLabel(Bounds(16, 792, 200, 24), "Genres", TypographyRole::kTitle);
GridLayout genreGrid(Bounds(16, 824, kWidth - 32, 0), 90, 110);
...
void setup() {
  ...
  playFab = IconButton<RGB565>(Bounds(kWidth - 72, 712, 56, 56), drawPlus<RGB565>,
  ...
  // Added last so it draws (and hit-tests) on top of whatever it overlaps
  // while shown.
  screen.addWidget(keyboard);
